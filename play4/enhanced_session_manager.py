...
class SessionManager:
    ...
    def create_session(self, videos: List[str], name: Optional[str] = None) -> SessionData:
    ...
    def cleanup_old_sessions(self, max_age_days: int = 7, keep_recent: int = 5):
        ...
        for session in sessions:
            if (session.session_id not in to_keep and 
    ...
    def interactive_session_select(self) -> Optional[SessionData]:
        ...
        for i, session in enumerate(sessions):
            ...
            print(f"    📊 Progress: {progress:.1f}% ({session.current_index}/{session.total_songs} songs)")
...
def load_or_create_session(config, videos: List[str], force_new: bool = False) -> Tuple[List[str], int, Optional[str]]:
